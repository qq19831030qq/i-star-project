...
class mysql extends db implements Idb{
    ...
    private static function init(){
        if(self::$_conn == null){
            try{
                ...
                self::$_conn = mysqli_connect($s['host'], $s['user'], $s['pass'], $app_mysql['dbname'], $s['port']);
    ...
    private static function command($sql, $conn = null){
        ...
        if (!$conn) {    // 
            throw new Exception('数据库连接不存在，连接变量为`'.get_class($conn).'`',CORE_DB_MYSQL_EC_NO_CONNECT);
        }
        try{
    ...
    private static function _error($sql, $conn){
        self::init();
        Error::logError(CORE_DB_MYSQL_EC_SYSTEM_ERROR, ERROR_SHOW, array('sql语句'=>$sql,'mysql错误代码'=>mysqli_errno($conn),'mysql错误内容'=>mysqli_error($conn)));
    } 

    public static function commit() {
        return mysqli_commit(self::$_conn);
    }

    public static function rollback() {
        return mysqli_rollback(self::$_conn);
    }
}

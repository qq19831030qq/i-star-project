...
define('PARAM_PHONE',    0x04000000); // 手机或电话类型

// 类型域
define('PARAM_TEXT',    PARAM_STRING ^ PARAM_STRIPTAGS);
define('PARAM_ARR_INT', PARAM_ARRAY ^ PARAM_INT);
...
class validator{
  ...
  */
  ...
  public static function get_param_uint($value,$argv = PARAM_UINT){
    if(ctype_digit($value) || is_int($value) || is_numeric($value)){
      if($argv & PARAM_OBJID){
        //验证是否对象ID,PARAM_TID
      }
  ...
  public static function get_param_mobile($mobilephone){
      ...
      return null;
      //throw new Exception('检验的变量不是有效的手机号码,变量值为`'.$mobilephone.'`',CORE_VALIDATOR_EC_NOT_MOBILE);
  }

  public static function get_param_phone($phone){
      $exp = "/^[0-9]{3,4}\-[0-9]{3,8}$|[0-9]{3,8}$|\([0-9]{3,4}\)[0-9]{3,8}$|0{0,1}1[34568][0-9]{9}$/"; 
      if(preg_match($exp,$phone)) return $phone; 
      return null;
      //throw new Exception('检验的变量不是有效的电话或手机号码,变量值为`'.$phone.'`',CORE_VALIDATOR_EC_NOT_MOBILE);
  }
}

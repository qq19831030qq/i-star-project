 ...
 * 调用方法：
 * 方法一：
 * $ic = new thumbnail('old.jpg','afterCrop.jpg');
 * $ic->Crop(120,80,2);
 * $ic->SaveImage();
 * //$ic->SaveAlpha();将补白变成透明像素保存
 * $ic->destory();
 *方法二：
 * $im = new thumbnail('old.jpg');
 * $im->SetDstFile('afterCrop.jpg');
 * $im->Crop($v['width'],$v['height'],4);
 * $im->SaveImage();
 * //$ic->SaveAlpha();将补白变成透明像素保存
 * $ic->destory();
 *
...
class thumbnail {
    ...
    //生成缩略图的大小
    function getsize($orgwidth,$orgheight,$size){
        if($orgwidth && $orgheight && $size){
            $settingsize = array(
                '1'=> array(
                    'width' => 640,
                    'height' => 680
            ...
            $ratio_w = 1.0 * $settingsize[$size]['width'] / $orgwidth;
            $ratio_h = 1.0 * $settingsize[$size]['height'] / $orgheight;
            $ratio = $ratio_w > $ratio_h ? $ratio_h : $ratio_w;
            $tmp_w = (int)($orgwidth * $ratio);
            $tmp_h = (int)($orgheight * $ratio);
            return array('width'=>$tmp_w,'height'=>$tmp_h);
        }else{
